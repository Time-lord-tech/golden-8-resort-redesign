            ...
            <div className="flex-1 p-8 border-b md:border-b-0 md:border-r border-[#1a365d]">
              ...
              <div className="flex gap-4">
                {filterStatus === 'pending' && (
                  <>
                    <button onClick={() => group.forEach(g => handleAction(g.id, 'reserved'))} className="flex-1 bg-emerald-500 hover:bg-emerald-600 text-white font-bold py-3.5 rounded-xl transition-colors flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(16,185,129,0.3)]">
                      <CheckCircle size={20} /> Verify Form & Reserve
                    </button>
                    <button onClick={() => group.forEach(g => handleAction(g.id, 'cancelled'))} className="px-8 bg-red-500/10 hover:bg-red-500/20 text-red-500 border border-red-500/20 font-bold py-3.5 rounded-xl transition-colors">
                      Reject
                    </button>
                  </>
                )}
                {filterStatus === 'reserved' && (
                  <button onClick={() => group.forEach(g => handleAction(g.id, 'checked_in'))} className="flex-1 bg-blue-500 hover:bg-blue-600 text-white font-bold py-3.5 rounded-xl transition-colors flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(59,130,246,0.3)]">
                    <CheckCircle size={20} /> Guest Checked In
                  </button>
                )}
                {filterStatus === 'checked_in' && (
                  <button onClick={() => group.forEach(g => handleAction(g.id, 'completed'))} className="flex-1 bg-slate-800 hover:bg-slate-700 text-white font-bold py-3.5 rounded-xl transition-colors flex items-center justify-center gap-2">
                    Finish & Check Out
                  </button>
                )}
              </div>
